...
const mockBudgetContext = {
  categories: [
    {
      ...
      name: "Test Category",
      associated_funds: [
        { id: "fund-1", name: "Disponible" },
        { id: "fund-2", name: "Ahorros" },
      ],
    },
  ],
  periods: [
  ...
  getPeriodById: jest.fn((id) =>
    mockBudgetContext.periods.find((p) => p.id === id)
  ),
  getFundById: jest.fn((id) =>
...
jest.mock("../source-fund-selector", () => ({
  SourceFundSelector: ({ selectedSourceFund, onSourceFundChange }: any) => (
    <select
