...
import { InterestRateForm, type InterestRateFormData } from "./interest-rate-form";
...
import { convertRate, getConversionDisplay } from "@/lib/interest-rate-calculations";
...
export function InterestRateCalculator({ initialScenario }: InterestRateCalculatorProps) {
  ...
  const [savedScenarios, setSavedScenarios] = useState<InterestRateScenarioWithConversions[]>([]);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorData.code === "TABLES_NOT_FOUND") {
          ...
          const migrationResponse = await fetch("/api/migrate-interest-rate-simulator", {
            method: "POST",
          });
  ...
  const handleSaveScenario = async (name: string, notes?: string) => {
    try {
      ...
      if (loadedScenarioId) {
        ...
        response = await fetch(`/api/interest-rate-scenarios/${loadedScenarioId}`, {
          method: "PUT",
      ...
      const savedScenario: InterestRateScenarioWithConversions = await response.json();
  ...
  const handleLoadScenario = (scenario: InterestRateScenarioWithConversions) => {
    setFormData({
  ...
  const handleDeleteScenario = async (scenarioId: string) => {
    try {
      const response = await fetch(`/api/interest-rate-scenarios/${scenarioId}`, {
        method: "DELETE",
      });
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "No se pudo eliminar el escenario",
        variant: "destructive",
      });
    }
  };
    ...
    <Tabs defaultValue="calculator" className="w-full">
      ...
      <TabsContent value="calculator" className="space-y-6">
        ...
        <div className="flex justify-end gap-2">
          ...
          >
            <RefreshCw className={`h-4 w-4 mr-2 ${isLoadingScenarios ? "animate-spin" : ""}`} />
            Actualizar lista
          </Button>
          <SaveRateDialog
