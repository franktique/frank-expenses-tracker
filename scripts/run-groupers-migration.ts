...
async function main() {
  ...
  if (dbClientError || !connected) {
    console.error(
      'Failed to create or connect database client:',
      dbClientError || 'Not connected'
    );
    process.exit(1);
  }
  ...
  try {
    ...
    for (const statement of statements) {
      console.log(
        `Executing: ${statement.substring(0, 100)}${statement.length > 100 ? '...' : ''}`
      );
      // Add back the semicolon if it's not a comment-only line
      const queryToExecute = statement.startsWith('--')
        ? statement
        : `${statement};`;
    ...
    console.log(
      'SQL script executed successfully! Tables for groupers should now exist.'
    );
  } catch (error) {
