  ...
  return (
    <Card>
      ...
      <CardContent className="p-0 sm:p-6">
        ...
        <div className="h-[60vh] max-h-[900px] min-h-[400px] w-full overflow-hidden">
          <ResponsiveContainer
            width="100%"
            height="100%"
            minHeight={350}
    ...
    if (excludedPaymentMethods.length === 0) {
      ...
      if (rawData.length > 0) {
        ...
        const filteredRawData = rawData.filter(
          (item) => !excludedPaymentMethods.includes(item.payment_method)
        );

        // Group by category and sum amounts
        const categoryTotals = filteredRawData.reduce(
          (acc, item) => {
