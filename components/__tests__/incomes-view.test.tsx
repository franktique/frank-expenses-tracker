...
describe("IncomesView", () => {
  ...
  describe("Initial Rendering", () => {
    ...
    it("shows no periods warning when no periods available", () => {
      ...
      expect(screen.getByText("No hay periodos disponibles")).toBeInTheDocument();
      expect(screen.getByText(/Para registrar ingresos, primero debes crear un periodo/)).toBeInTheDocument();
    ...
    it("displays active period information", () => {
      ...
      expect(screen.getByText("Ingresos del periodo: Enero 2024")).toBeInTheDocument();
    });
    ...
    it("shows all incomes when no active period", () => {
      ...
      expect(screen.getByText("Historial de todos los ingresos registrados")).toBeInTheDocument();
    });
  });
  ...
  describe("Income Data Display", () => {
    ...
    it("shows empty state when no incomes exist", () => {
      ...
      expect(screen.getByText("No hay ingresos registrados. Agrega un nuevo ingreso para comenzar.")).toBeInTheDocument();
    });
  });
  ...
  describe("Add Income Dialog", () => {
    ...
    it("shows default fund information", () => {
      ...
      expect(screen.getByText(/Si no seleccionas un fondo, se asignará al fondo "Disponible"/)).toBeInTheDocument();
    });
    ...
    it("shows active period fallback information", () => {
      ...
      expect(screen.getByText("Se usará el periodo activo: Enero 2024")).toBeInTheDocument();
    });
  });
  ...
  describe("Income Creation", () => {
    ...
    it("handles creation errors", async () => {
      const mockAddIncome = jest.fn().mockRejectedValue(new Error("Database error"));
  ...
  describe("Income Editing", () => {
    it("opens edit dialog when edit button is clicked", () => {
      ...
      expect(screen.getByText("Actualiza los detalles del ingreso")).toBeInTheDocument();
    });
    ...
    it("updates income successfully", async () => {
      ...
      renderWithProviders(<IncomesView />, {
        budgetContextValue: { ...mockBudgetContext, updateIncome: mockUpdateIncome },
      });
  ...
  describe("Income Deletion", () => {
    ...
    it("deletes income successfully", async () => {
      ...
      renderWithProviders(<IncomesView />, {
        budgetContextValue: { ...mockBudgetContext, deleteIncome: mockDeleteIncome },
      });
    ...
    it("handles deletion errors", async () => {
      const mockDeleteIncome = jest.fn().mockRejectedValue(new Error("Cannot delete"));
  ...
  describe("Loading States", () => {
    it("disables buttons during submission", async () => {
      const mockAddIncome = jest.fn().mockImplementation(() => new Promise(resolve => setTimeout(resolve, 1000)));
  ...
  describe("Accessibility", () => {
    it("has proper ARIA labels and roles", () => {
      ...
      const actionButtons = screen.getAllByRole("button", { name: /editar|eliminar/i });
