...
describe('Source Fund Migration Scenarios', () => {
  ...
  describe('Migration with Legacy Fund Relationships', () => {
    it('should fallback to legacy fund_id when no category_fund_relationships exist', async () => {
      const mockExpenses = [
        {
          ...
          category_id: 'cat-1',
          amount: 100,
          description: 'Legacy expense 1',
        },
        {
          id: 'exp-2',
          category_id: 'cat-2',
          amount: 150,
          description: 'Legacy expense 2',
        },
      ];
      ...
      const result =
        await sql`UPDATE expenses SET source_fund_id = (SELECT c.fund_id FROM categories c WHERE c.id = expenses.category_id) WHERE source_fund_id IS NULL RETURNING id, category_id, source_fund_id`;
  ...
  describe('Migration Error Scenarios', () => {
    ...
    it('should handle categories with no fund relationships', async () => {
      const mockExpenses = [
        {
          id: 'exp-1',
          category_id: 'cat-no-funds',
          amount: 75,
          description: 'No funds expense',
        },
      ];
    ...
    it('should handle partial migration failures gracefully', async () => {
      const mockExpenses = [
        ...
        },
        {
          ...
          category_id: 'cat-2',
          amount: 150,
          description: 'Another good expense',
        },
      ];
  ...
  describe('Migration Status Tracking', () => {
    it('should accurately track migration progress', async () => {
      ...
      const preMigration =
        await sql`SELECT * FROM check_expense_source_fund_migration_status()`;
      const postMigration =
        await sql`SELECT * FROM check_expense_source_fund_migration_status()`;
