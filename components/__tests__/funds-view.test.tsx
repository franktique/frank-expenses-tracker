...
jest.mock("@/lib/fund-error-handling", () => ({
  ...
  validateFundForm: jest.fn((data, schema) => ({
    ...
    errors: data.name && data.name.length > 0 ? [] : [{ field: "name", message: "Nombre es requerido" }],
  })),
...
jest.mock("@/components/fund-error-display", () => ({
  ...
  FundBalanceErrorDisplay: ({ fundName, error, onRecalculate, onDismiss }: any) => (
    <div data-testid="fund-balance-error-display">
      <span>{fundName}: {error}</span>
...
jest.mock("@/components/fund-empty-states", () => ({
  ...
  FundsTableSkeleton: () => <div data-testid="funds-table-skeleton">Loading...</div>,
...
describe("FundsView", () => {
  ...
  describe("Fund Data Display", () => {
    ...
    it("shows 'Sin descripción' for funds without description", () => {
      const fundsWithoutDescription = [
        { ...mockFunds[0], description: "" },
      ];

      renderWithProviders(<FundsView />, {
        budgetContextValue: { ...mockBudgetContext, funds: fundsWithoutDescription },
      });
  ...
  describe("Fund Creation", () => {
    ...
    it("shows validation errors for invalid data", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("fund-validation-error-display")).toBeInTheDocument();
        expect(screen.getByText("Nombre es requerido")).toBeInTheDocument();
      });
    });

    it("handles creation errors", async () => {
      const mockAddFund = jest.fn().mockRejectedValue(new Error("Fund already exists"));
  ...
  describe("Fund Editing", () => {
    it("opens edit dialog when edit button is clicked", () => {
      ...
      expect(screen.getByText("Actualiza la información del fondo")).toBeInTheDocument();
    });
    ...
    it("populates edit form with existing fund data", () => {
      ...
      expect(screen.getByDisplayValue("Dinero en efectivo")).toBeInTheDocument();
    ...
    it("shows note about date modification restriction", () => {
      ...
      expect(screen.getByText(/La fecha de inicio no se puede modificar/)).toBeInTheDocument();
    });
    ...
    it("updates fund successfully", async () => {
      ...
      renderWithProviders(<FundsView />, {
        budgetContextValue: { ...mockBudgetContext, updateFund: mockUpdateFund },
      });
  ...
  describe("Fund Deletion", () => {
    it("opens delete dialog when delete button is clicked", () => {
      ...
      expect(screen.getByText(/No podrás eliminar el fondo si tiene categorías/)).toBeInTheDocument();
    });
    ...
    it("deletes fund successfully", async () => {
      ...
      renderWithProviders(<FundsView />, {
        budgetContextValue: { ...mockBudgetContext, deleteFund: mockDeleteFund },
      });
    ...
    it("handles deletion errors", async () => {
      const mockDeleteFund = jest.fn().mockRejectedValue(new Error("Fund has associated transactions"));
  ...
  describe("Balance Recalculation", () => {
    it("opens recalculate dialog when calculator button is clicked", () => {
      ...
      expect(screen.getByText(/Ingresos asignados al fondo/)).toBeInTheDocument();
    });
    ...
    it("recalculates balance successfully", async () => {
      ...
      renderWithProviders(<FundsView />, {
        budgetContextValue: { ...mockBudgetContext, recalculateFundBalance: mockRecalculateFundBalance },
      });
    ...
    it("displays balance error in table", async () => {
      const mockRecalculateFundBalance = jest.fn().mockRejectedValue(new Error("Network error"));
      ...
      await waitFor(() => {
        expect(screen.getByTestId("fund-balance-error-display")).toBeInTheDocument();
      });
    });
  });
  ...
  describe("Error Handling", () => {
    it("displays general errors", async () => {
      const mockAddFund = jest.fn().mockRejectedValue(new Error("General error"));
    ...
    it("allows dismissing errors", async () => {
      ...
      expect(screen.queryByTestId("fund-error-display")).not.toBeInTheDocument();
    });
  ...
  describe("Accessibility", () => {
    it("has proper ARIA labels and roles", () => {
      ...
      expect(screen.getByRole("button", { name: /nuevo fondo/i })).toBeInTheDocument();
