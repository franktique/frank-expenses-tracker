...
async function listIncomes() {
  ...
  incomes.forEach((income, index) => {
    console.log(
      `${index + 1}. ${income.id} | ${income.date} | ${income.description} | $${income.amount} | ${income.period_id || 'N/A'}`
    );
  });
        ...
        case '2':
          ...
          if (incomes.length > 0) {
            const indexAnswer = await new Promise((resolve) => {
              rl.question(
                '\nEnter the index number of the income to delete (or 0 to cancel): ',
                resolve
              );
            });
            ...
            if (index >= 0 && index < incomes.length) {
              const confirmed = await new Promise((resolve) => {
                rl.question(
                  `Are you sure you want to delete income "${incomes[index].description}" with amount $${incomes[index].amount}? (y/n): `,
                  (answer) => {
