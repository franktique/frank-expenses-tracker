        ...
        setGroupers(
          groupers.map((g) =>
            g.id === currentGrouper.id
              ? {
                  ...updatedGrouper,
                  category_count: currentGrouper.category_count,
                }
              : g
          )
        );
    ...
    <div className="container mx-auto py-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Gestione los agrupadores para organizar sus categorías de
            presupuesto.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <div className="py-4 text-center">Cargando agrupadores...</div>
          ) : groupers.length === 0 ? (
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          ...
          <p>
            ¿Está seguro de que desea eliminar el agrupador "
            {currentGrouper?.name}"?
            {currentGrouper && currentGrouper.category_count > 0 && (
              <span className="mt-2 block text-red-500">
                Este agrupador contiene {currentGrouper.category_count}{' '}
                categorías que serán desasociadas.
              </span>
            )}
          </p>
          <DialogFooter>
