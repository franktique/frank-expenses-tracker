...
export function IncomesView() {
  const { periods, incomes, activePeriod, addIncome, updateIncome, deleteIncome } = useBudget()
  ...
  const [newIncomePeriod, setNewIncomePeriod] = useState(activePeriod?.id || "")
  ...
  const handleEditIncome = async () => {
    if (!editIncome || !editIncome.date || !editIncome.description.trim() || !editIncome.amount) {
      toast({
        title: "Error",
        description: "Fecha, descripción y monto son obligatorios",
        variant: "destructive",
    ...
    return (
      <div className="space-y-6">
        ...
        >
          ...
          <AlertDescription>
            <p>Para registrar ingresos, primero debes crear un periodo en la sección de Periodos.</p>
            <Button variant="outline" className="mt-2" onClick={() => (window.location.href = "/periodos")}>
              Ir a Periodos
            </Button>
          </AlertDescription>
        </Alert>
        ...
        <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Ingresa los detalles del nuevo ingreso</DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                <Label htmlFor="period">Periodo *</Label>
                <Select value={newIncomePeriod} onValueChange={setNewIncomePeriod} disabled={!periods.length}>
                ...
                {!newIncomePeriod && activePeriod && (
                  <p className="text-xs text-muted-foreground">Se usará el periodo activo: {activePeriod.name}</p>
                )}
              </div>
              <div className="grid gap-2">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    ...
                    >
                      ...
                      {newIncomeDate ? formatDate(newIncomeDate) : <span>Selecciona una fecha</span>}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0">
                    <Calendar mode="single" selected={newIncomeDate} onSelect={setNewIncomeDate} initialFocus />
                  </PopoverContent>
                </Popover>
              </div>
              <div className="grid gap-2">
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Actualiza los detalles del ingreso</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Select
                ...
                onValueChange={(value) => setEditIncome((prev) => (prev ? { ...prev, periodId: value } : null))}
            ...
            <div className="grid gap-2">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    <CalendarIcon className="mr-2 h-4 w-4" />
                    {editIncome?.date ? formatDate(editIncome.date) : <span>Selecciona una fecha</span>}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0">
                  <Calendar
                    ...
                    onSelect={(date) => setEditIncome((prev) => (prev ? { ...prev, date } : null))}
                    initialFocus
                  />
                </PopoverContent>
              </Popover>
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) => setEditIncome((prev) => (prev ? { ...prev, description: e.target.value } : null))}
              />
            </div>
            <div className="grid gap-2">
      ...
      <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleDeleteIncome} disabled={isSubmitting}>
