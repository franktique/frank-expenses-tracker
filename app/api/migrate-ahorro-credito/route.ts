  ...
  try {
    ...
    if (!efectivoColumnExists) {
      try {
        ...
        migrationResults.push({
          step: "add_ahorro_efectivo_column",
          success: false,
          message: "Failed to add ahorro_efectivo_amount column: " + (error as Error).message,
        });
        throw error;
      }
    } else {
    ...
    if (!creditoColumnExists) {
      try {
        ...
        migrationResults.push({
          step: "add_ahorro_credito_column",
          success: false,
          message: "Failed to add ahorro_credito_amount column: " + (error as Error).message,
        });
        throw error;
      }
    } else {
    ...
    if (!efectivoColumnExists) {
      try {
        ...
        console.log(`✓ Migrated expected_savings to ahorro_efectivo_amount for ${rowCount} rows`);
      } catch (error) {
        migrationResults.push({
          step: "migrate_expected_savings",
          success: false,
          message: "Failed to migrate expected_savings: " + (error as Error).message,
        });
        throw error;
      }
    } else {
    ...
    const efectivoConstraintExists =
      efectivoConstraintCheck.length > 0 ? efectivoConstraintCheck[0].exists : false;
    ...
    if (!efectivoConstraintExists) {
      try {
        ...
        migrationResults.push({
          step: "add_ahorro_efectivo_constraint",
          success: false,
          message: "Failed to add ahorro_efectivo constraint: " + (error as Error).message,
        });
        throw error;
      }
    } else {
    ...
    const creditoConstraintExists =
      creditoConstraintCheck.length > 0 ? creditoConstraintCheck[0].exists : false;
    ...
    if (!creditoConstraintExists) {
      try {
        ...
        migrationResults.push({
          step: "add_ahorro_credito_constraint",
          success: false,
          message: "Failed to add ahorro_credito constraint: " + (error as Error).message,
        });
        throw error;
      }
    } else {
    ...
    if (!efectivoIndexExists) {
      try {
        ...
        migrationResults.push({
          step: "create_ahorro_efectivo_index",
          success: false,
          message: "Failed to create ahorro_efectivo index: " + (error as Error).message,
        });
        throw error;
      }
    } else {
    ...
    if (!creditoIndexExists) {
      try {
        ...
        migrationResults.push({
          step: "create_ahorro_credito_index",
          success: false,
          message: "Failed to create ahorro_credito index: " + (error as Error).message,
        });
        throw error;
      }
    } else {
    ...
    return NextResponse.json({
      ...
      details: {
        ...
        constraints: ["check_ahorro_efectivo_valid", "check_ahorro_credito_valid"],
        indexes: ["idx_simulation_budgets_ahorro_efectivo", "idx_simulation_budgets_ahorro_credito"],
        description:
