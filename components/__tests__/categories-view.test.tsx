...
jest.mock('../fund-category-relationship-indicator', () => ({
  FundCategoryRelationshipIndicator: () => (
    <div data-testid="relationship-indicator" />
  ),
}));
...
describe('CategoriesView', () => {
  ...
  describe('Add Category Dialog', () => {
    it('opens add category dialog when button is clicked', () => {
      ...
      expect(
        screen.getByPlaceholderText('Ej: Alimentación, Transporte, etc.')
      ).toBeInTheDocument();
    ...
    it('allows entering category name', () => {
      ...
      const nameInput = screen.getByPlaceholderText(
        'Ej: Alimentación, Transporte, etc.'
      );
      fireEvent.change(nameInput, {
        target: { value: 'Nueva Categoría Test' },
      });
  ...
  describe('Add Category Functionality', () => {
    it('creates new category successfully', async () => {
      ...
      mockFetch({
        success: true,
        category: { id: 'new-cat', name: 'Test Category' },
      });
    ...
    it('shows error when category name is empty', async () => {
      ...
      // Mock toast
      require('@/components/ui/use-toast').useToast = () => ({
        toast: mockToast,
      });
