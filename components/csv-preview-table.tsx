...
import { PaymentMethod, useBudget } from "@/context/budget-context-provider"
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            ...
            <span className="ml-2 font-medium text-green-600">{validItems.length} válidos</span>
            {invalidItems.length > 0 && (
              <span className="ml-2 font-medium text-red-600">{invalidItems.length} con errores</span>
            )}
          </p>
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {currentPageData.map((item, index) => (
                <TableRow key={item.id} className={item.hasError ? "bg-red-50 dark:bg-red-950/20" : ""}>
                  <TableCell>{start + index + 1}</TableCell>
                  <TableCell className={cn(item.errors.category ? "text-red-600" : "")}>
                    {editingId === item.id ? (
                      <Select 
                  ...
                  <TableCell className={cn(item.errors.date ? "text-red-600" : "")}>
                    {editingId === item.id ? (
                      <Popover>
                  ...
                  <TableCell className={cn(item.errors.payment_method ? "text-red-600" : "")}>
                    {editingId === item.id ? (
                      <Select 
                  ...
                  <TableCell className={cn(item.errors.period ? "text-red-600" : "")}>
                    {editingId === item.id ? (
                      <Select 
                  ...
                  <TableCell className={cn("text-right", item.errors.amount ? "text-red-600" : "")}>
                  ...
                  <TableCell className="text-center">
                    <div className="flex justify-center space-x-1">
                      {editingId === item.id ? (
                        <Button variant="ghost" size="icon" onClick={finishEditing}>
