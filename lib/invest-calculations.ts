...
// ============================================================================
// Target Income Calculation Functions
// ============================================================================

/**
 * Calculate required capital to generate a target monthly income from interest alone
 *
 * Formula:
 *   Monthly rate: r_m = (1 + EA/100)^(1/12) - 1
 *   Required capital: C = target_monthly_income / r_m
 *
 * @param annualRate - Annual effective rate as percentage (e.g., 8.25 for 8.25%)
 * @param targetMonthlyIncome - Desired monthly income in currency units
 * @returns Required capital amount
 */
export function calculateRequiredCapitalForMonthlyIncome(
  annualRate: number,
  targetMonthlyIncome: number
): number {
  if (annualRate <= 0 || targetMonthlyIncome <= 0) return 0;
  const monthlyRate = convertEAToMonthlyRate(annualRate);
  if (monthlyRate === 0) return 0;
  return roundToTwo(targetMonthlyIncome / monthlyRate);
}

/**
 * Calculate monthly income generated by a given capital at a given annual rate
 *
 * Formula:
 *   Monthly rate: r_m = (1 + EA/100)^(1/12) - 1
 *   Monthly income: I = capital * r_m
 *
 * @param annualRate - Annual effective rate as percentage (e.g., 8.25 for 8.25%)
 * @param capital - Capital amount invested
 * @returns Monthly income generated
 */
export function calculateMonthlyIncomeFromCapital(
  annualRate: number,
  capital: number
): number {
  if (annualRate <= 0 || capital <= 0) return 0;
  const monthlyRate = convertEAToMonthlyRate(annualRate);
  return roundToTwo(capital * monthlyRate);
}
