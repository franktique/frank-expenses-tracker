...
import { mockCategories, mockPeriods, mockBudgets, mockFunds } from "./fixtures/test-data";
...
describe("BudgetsView", () => {
  ...
  describe("Initial Rendering", () => {
    it("renders the budgets view with header", () => {
      ...
      expect(screen.getByText(/Configura los montos presupuestados/)).toBeInTheDocument();
    });
    ...
    it("shows no periods message when no periods available", () => {
      ...
      expect(screen.getByText(/Para configurar presupuestos, primero debes crear un periodo/)).toBeInTheDocument();
    });
  ...
  describe("Period Selection", () => {
    ...
    it("shows help text when no period is selected", () => {
      ...
      expect(screen.getByText("Selecciona un periodo para ver y configurar los presupuestos")).toBeInTheDocument();
    });
  });
  ...
  describe("Categories Table", () => {
    it("displays all categories in alphabetical order", () => {
      ...
      const categoryNames = screen.getAllByRole("cell").filter(cell => 
    ...
    it("shows empty state when no categories exist", () => {
      ...
      expect(screen.getByText("No hay categorías. Agrega categorías en la sección de Categorías.")).toBeInTheDocument();
    });
  });
  ...
  describe("Edit Budget Dialog", () => {
    ...
    it("displays correct category and period information in dialog", () => {
      ...
      expect(screen.getByText(/Define el monto presupuestado para/)).toBeInTheDocument();
    ...
    it("closes dialog when cancel button is clicked", () => {
      ...
      expect(screen.queryByText("Establecer Presupuesto")).not.toBeInTheDocument();
    });
  });
  ...
  describe("Budget Creation", () => {
    ...
    it("updates existing budget successfully", async () => {
      ...
      renderWithProviders(<BudgetsView />, {
        budgetContextValue: { ...mockBudgetContext, updateBudget: mockUpdateBudget },
      });
  ...
  describe("Accessibility", () => {
    ...
    it("has screen reader text for edit buttons", () => {
      ...
      expect(screen.getAllByText("Editar efectivo")).toHaveLength(mockCategories.length);
      expect(screen.getAllByText("Editar crédito")).toHaveLength(mockCategories.length);
    });
  });
});