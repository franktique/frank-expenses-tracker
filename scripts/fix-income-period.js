...
async function updateIncomeWithPeriod(incomeId, periodId) {
  ...
  if (response && !response.error) {
    ...
    console.error(
      ...
      response ? response.error : 'Error desconocido'
    );
    return false;
  }
}
...
async function main() {
  try {
    ...
    if (activePeriod) {
      ...
      console.log(
        'No se encontró un periodo activo. Usando el periodo más reciente como predeterminado.'
      );
    }
    ...
    sortedPeriods.forEach((period, index) => {
      console.log(
        `${index + 1}. ${period.name} (${period.is_open ? 'ACTIVO' : 'CERRADO'})`
      );
    });
    ...
    incomesWithoutPeriod.forEach((income, index) => {
      console.log(
        `${index + 1}. Fecha: ${income.date}, Descripción: ${income.description}, Monto: $${income.amount}`
      );
    });

    // Preguntar qué periodo usar
    const defaultPeriodIndex = activePeriod
      ? sortedPeriods.findIndex((p) => p.id === activePeriod.id)
      : 0;
    const periodAnswer = await new Promise((resolve) => {
      rl.question(
        `\nSeleccione el número del periodo al que desea asignar estos ingresos [${defaultPeriodIndex + 1}]: `,
        (answer) => {
          if (!answer.trim()) {
    ...
    const confirmation = await new Promise((resolve) => {
      rl.question(
        `¿Está seguro de que desea asignar ${incomesWithoutPeriod.length} ingreso(s) al periodo "${selectedPeriod.name}"? (s/n): `,
        (answer) => {
    ...
    for (const income of incomesWithoutPeriod) {
      const success = await updateIncomeWithPeriod(
        income.id,
        selectedPeriod.id
      );
      if (success) {
