...
checkRequirement(
  ...
  () => {
    ...
    // Check if CreditCardSelector is used in add form
    const hasAddFormSelector = expensesViewContent.includes(
      "selectedCreditCard={newExpenseCreditCard}"
    );

    // Check if CreditCardSelector is used in edit form
...
checkRequirement(
  ...
  () => {
    ...
    // Look for the pattern where amount field is followed by credit card field
    const addFormHasAmountField = expensesViewContent.includes(
      'htmlFor="amount">Monto *</Label>'
    );
    const addFormHasCreditCardField = expensesViewContent.includes(
      'htmlFor="credit-card">Tarjeta de Crédito (opcional)</Label>'
    );

    const editFormHasAmountField = expensesViewContent.includes(
      'htmlFor="edit-amount">Monto</Label>'
    );
    const editFormHasCreditCardField = expensesViewContent.includes(
      'htmlFor="edit-credit-card">Tarjeta de Crédito (opcional)</Label>'
    );

    // Check that credit card comes after amount in the file (simple position check)
    const amountPos = expensesViewContent.indexOf('htmlFor="amount">Monto *');
    const creditCardPos = expensesViewContent.indexOf(
      'htmlFor="credit-card">Tarjeta de Crédito (opcional)'
    );
    const addFormOrder = amountPos > 0 && creditCardPos > amountPos;
    ...
    const editCreditCardPos = expensesViewContent.indexOf(
      'htmlFor="edit-credit-card">Tarjeta de Crédito (opcional)'
    );
    const editFormOrder =
      editAmountPos > 0 && editCreditCardPos > editAmountPos;

    console.log(
      `  - Add form has amount: ${addFormHasAmountField ? "✅" : "❌"}`
    );
    console.log(
      `  - Add form has credit card: ${addFormHasCreditCardField ? "✅" : "❌"}`
    );
    console.log(`  - Add form order: ${addFormOrder ? "✅" : "❌"}`);
    console.log(
      `  - Edit form has amount: ${editFormHasAmountField ? "✅" : "❌"}`
    );
    console.log(
      `  - Edit form has credit card: ${
        editFormHasCreditCardField ? "✅" : "❌"
      }`
    );
    console.log(`  - Edit form order: ${editFormOrder ? "✅" : "❌"}`);

    return (
      addFormHasAmountField &&
      addFormHasCreditCardField &&
      addFormOrder &&
      editFormHasAmountField &&
      editFormHasCreditCardField &&
      editFormOrder
    );
  }
