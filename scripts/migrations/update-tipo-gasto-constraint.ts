...
export async function migrateUpdateTipoGastoConstraint() {
  try {
    ...
    if (!columnExists) {
      console.log("Column tipo_gasto does not exist. Skipping constraint update.");
      return { success: true, message: "Column does not exist, skipping migration" };
    }
    ...
    console.log("✓ Successfully added updated check constraint with E (Eventual)");
