...
export function RefreshTemplateDialog({
      ...
      if (data.success && data.simulations) {
        ...
        const simulationsWithSubgroups = await Promise.all(
          data.simulations.map(async (sim: any) => {
            try {
              const subgroupsResponse = await fetch(`/api/simulations/${sim.id}/subgroups`);
              const subgroupsData = await subgroupsResponse.json();
              return {
                id: sim.id,
                name: sim.name || `Simulation ${sim.id}`,
                hasSubgroups: subgroupsData.success && subgroupsData.subgroups?.length > 0,
              };
            } catch {
              return {
                id: sim.id,
  ...
  const handleRefresh = async () => {
    ...
    try {
      const response = await fetch(`/api/subgroup-templates/${template.id}/refresh`, {
        method: "POST",
        headers: {
      ...
      if (data.success) {
        toast({
          ...
          description: data.message || "Template has been refreshed successfully",
        });
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to refresh template",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Update this template with subgroups and categories from another simulation.
            This will replace all existing template data with the source simulation data.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900 p-3 rounded-md text-sm">
            <p className="font-medium text-amber-800 dark:text-amber-400 mb-1">
              ⚠️ This action will replace the template
            </p>
            <ul className="list-disc list-inside text-amber-700 dark:text-amber-500 text-xs space-y-1">
